...
inline fun <reified T> CtClass.injectMetaData(a: List<T>?, methodName: String) {
    ...
    getDeclaredMethod(methodName)
        .insertAfter(buildString {
            if (a.first() is IExtensionMetaData) {
                appendln("com.spacecraft.hybrid.ExtensionMetaData extension;")
                a.filterIsInstance<IExtensionMetaData>().forEach { extension ->
                    appendln("extension = new com.spacecraft.hybrid.ExtensionMetaData(\"${extension.name}\", \"${extension.classname}\");")
                    extension.methods.forEach { method ->
                        ...
                        appendln(
                            "extension.addMethod(\"${method.name}\", ${method.isInstanceMethod}, " +
                                    "com.spacecraft.hybrid.annotation.Extension.Mode.${method.mode.name}, com.spacecraft.hybrid.annotation.Extension.Type.${method.type.name}, " +
                                    "com.spacecraft.hybrid.annotation.Extension.Access.${method.access.name}, com.spacecraft.hybrid.annotation.Extension.Normalize.${method.normalize.name}, " +
                                    "com.spacecraft.hybrid.annotation.Extension.Multiple.${method.multiple.name}, ${alias}, " +
            ...
            } else if (a.first() is WidgetMetaData) {
                appendln("com.spacecraft.hybrid.Widget widget;")
                a.filterIsInstance<WidgetMetaData>().forEach { widget ->
                    appendln("widget = new com.spacecraft.hybrid.Widget(\"${widget.name}\", ${widget.classname}.class);")
