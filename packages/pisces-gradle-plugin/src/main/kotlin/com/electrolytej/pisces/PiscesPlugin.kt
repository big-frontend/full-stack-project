...
class PiscesPlugin : ScanClassPlugin() {
    override fun getName() = "pisces"
    ...
    override fun onScanClass(info: ClassInfo) {
        try {
            if (!info.canonicalName.contains("") && !info.canonicalName.contains("com.spacecraft.hybrid")) return
            info.mather?.let {
                classPool.appendClassPath(it.absolutePath)
            }
            processors.forEach {
        ...
        for (processor in processors) {
            ...
            Injector.injectCode(processor.modifiedClassInfo) { where, inputStream ->
                ...
                try {
                    ...
                    P.error("[${processor.javaClass.simpleName}] could not insert code in ${modifiedCtClass.name};  ${processor.modifiedClassInfo?.mather?.absolutePath}$e")
                    byteArrayOf()
                } finally {
